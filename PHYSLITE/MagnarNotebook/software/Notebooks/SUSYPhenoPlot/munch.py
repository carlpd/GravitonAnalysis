...
class munch: 
    ...
    def __init__(s, cmd=[], optD={}):
        ...
        # ====================== EXECUTE 
        if 'showHelp' in s.cmd: s.showHelp()
    ...
    def getHist(s):
        cl = s.hDef.Clone(s.hDef.GetName()+"_local")
        cl.SetDirectory(0)
        return cl
    def get2DHist(s):
        cl = s.h2.Clone(s.h2.GetName()+"_local")
        cl.SetDirectory(0)
        return cl
    def get2DCont(s):
        return s.hgr.copy()
    def getCont(s):
        return s.hgr_cont.copy()
    def getCanv(s):
        ca = s.c1.DrawClone(s.c1.GetName()+"_local")
        return ca
    # ##########
    def Plot2D(s):
        ...
        tr = ROOT.TRandom(int(time.time()))
        rnd = int(tr.Uniform(0,100))
        s.c1 = ROOT.TCanvas('sc1_%i'%rnd,'sc1_%i'%rnd,s.dict['canv_xpos'],s.dict['canv_ypos'],s.dict['canv_xwidth'],s.dict['canv_ywidth'])
        ...
        hDef = ROOT.TH2F('hDef_%i'%rnd, s.title, int(int(s.xrangemax - s.xrangemin)/50), s.xrangemin, s.xrangemax, int(s.yrangemax - s.yrangemin), s.yrangemin, s.yrangemax)
        hDef.SetDirectory(0)
        ...
        for cont in s.conts:
            ...
            ## Experimental (by Eirik)
            # minstepx = 1e6
            # for i in range(len(x)-1):
            #     diff = x[i+1] - x[i]
            #     if diff > 0 and diff < minstepx:
            #         minstepx = diff
            # minstepy = 1e6
            # for i in range(len(y)-1):
            #     diff = y[i+1] - y[i]
            #     if diff > 0 and diff < minstepy:
            #         minstepy = diff

            
            # h2 = ROOT.TH2D('h2_%i'%rnd, s.title, int(int(s.xrangemax - s.xrangemin)/int(minstepx)), s.xrangemin, s.xrangemax, int(int(s.yrangemax - s.yrangemin)/int(minstepy)), s.yrangemin, s.yrangemax)
            # for i in range(len(x)):
            #     ibin = h2.FindBin(x[i],y[i])
            #     print("ibin: %i : x = %i, y = %i, cont = %.2f"%(ibin,x[i],y[i],s.contval[cont][i]))
            #     ibin2 = h2.Fill(x[i],y[i],s.contval[cont][i])
            #     print("ibin2 = %i"%ibin2)
            # s.h2 = h2
            ...
            s.hgr_cont[cont].SetDirectory(0)
